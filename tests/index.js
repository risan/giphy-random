...
test('can set the uri property', () => {
  const giphyRandom = new GiphyRandom({ apiKey: API_KEY, uri: 'https://foo.bar' });
...
test('can get random GIF', () => {
  ...
  return expect(giphyRandom.get())
    .resolves.toEqual(SUCCESS_RESPONSE.data);
});
...
test('can get random GIF with tag parameter', () => {
  ...
  return expect(giphyRandom.get({ tag: 'bar' }))
    .resolves.toEqual(SUCCESS_RESPONSE.data);
});
...
test('can get random GIF with rating parameter', () => {
  ...
  return expect(giphyRandom.get({ rating: 'PG' }))
    .resolves.toEqual(SUCCESS_RESPONSE.data);
});
